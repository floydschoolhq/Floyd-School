...
const StudentRoster = () => {
  ...
  const [promoteBatchId, setPromoteBatchId] = useState('all');
  const [targetGrade, setTargetGrade] = useState('Grade 11');
  ...
  const handlePromoteSubmit = async (e) => {
    e.preventDefault();
    const studentsToPromote = students.filter(s => promoteBatchId === 'all' || s.batchId?._id === promoteBatchId);
    if (studentsToPromote.length === 0) {
      addToast('No students available in selected batch to promote', 'error');
      return;
    }

    setSubmitting(true);
    try {
      const studentIds = studentsToPromote.map(s => s._id);
      const res = await api.post('/partner-school/promote-students', {
        studentIds,
        targetGrade,
        targetAcademicYear
      });
      addToast(res.data.message || 'Students promoted successfully!', 'success');
      setShowPromoteModal(false);
      fetchData();
    } catch (error) {
      addToast(error.response?.data?.message || 'Promotion failed', 'error');
    } finally {
      setSubmitting(false);
    }
  };
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 card-modern rounded-xl p-4">
        ...
        <div className="flex items-center space-x-2">
          <button
            ...
            className="btn-modern-secondary px-3 py-1.5 rounded-lg text-xs font-semibold flex items-center space-x-1.5 text-slate-700 hover:text-slate-900"
          >
      ...
      {showPromoteModal && (
        <div className="fixed inset-0 z-50 bg-slate-900/30 backdrop-blur-2xs flex items-center justify-center p-4">
          <div className="bg-white border border-slate-200 rounded-xl p-6 w-full max-w-md relative shadow-lg space-y-4">
            <div className="flex items-center justify-between border-b border-slate-100 pb-3">
              <div>
                <h3 className="text-base font-bold text-slate-900">Promote Class / Academic Year</h3>
                <p className="text-xs text-slate-500">Batch-promote students to the next grade and academic year.</p>
              </div>
              <button onClick={() => setShowPromoteModal(false)} className="text-slate-400 hover:text-slate-800">
                <X size={18} />
              </button>
            </div>
            ...
            <form onSubmit={handlePromoteSubmit} className="space-y-3">
              <div>
                <label className="block text-xs font-semibold text-slate-700 mb-1">Select Current Batch to Promote</label>
                <select
                  value={promoteBatchId}
                  onChange={(e) => setPromoteBatchId(e.target.value)}
                  className="w-full bg-white border border-slate-200 rounded-lg px-2.5 py-1.5 text-xs text-slate-900 focus:outline-none"
                >
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full bg-white border border-slate-200 rounded-lg px-2.5 py-1.5 text-xs text-slate-900 focus:outline-none focus:border-slate-800"
                />
              </div>

              <div>
                <label className="block text-xs font-semibold text-slate-700 mb-1">Target Academic Year</label>
                <input
                  type="text"
                  required
              ...
              <div className="bg-amber-50 border border-amber-200 rounded-lg p-3 text-xs text-amber-800">
                This will update the academic year and grade of all selected students in this cohort while preserving their permanent records and historical attendance.
              </div>
              ...
              <button
                ...
                className="w-full py-2.5 btn-modern-primary rounded-lg text-xs font-semibold transition-all disabled:opacity-50"
              >
                {submitting ? 'Promoting Students...' : 'Confirm Cohort Promotion'}
              </button>
            </form>
          </div>
        </div>
