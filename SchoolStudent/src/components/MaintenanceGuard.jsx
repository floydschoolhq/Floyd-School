...
export default function MaintenanceGuard({ children }) {
  ...
  const checkStatus = async () => {
    setChecking(true);
    try {
      const res = await api.get('/public/maintenance-status?portal=schoolStudent');
      ...
      // If network fails or endpoint down, don't hard block
      setMaintenance(null);
    } finally {
      setLoading(false);
      setChecking(false);
        ...
        <div className="max-w-md w-full bg-slate-800/80 border border-amber-500/30 rounded-2xl p-8 shadow-2xl backdrop-blur-md space-y-6">
          ...
          <div className="space-y-2">
            ...
            <p className="text-xs text-slate-300 leading-relaxed">
              {maintenance.message || 'We are performing scheduled maintenance and updates to improve your offline classroom experience.'}
            </p>
          </div>
          ...
          {maintenance.endTime && (
            <div className="bg-slate-900/60 border border-slate-700/60 rounded-xl p-3 text-xs text-slate-300 flex items-center justify-center gap-2">
              ...
              <span>Expected return by: <strong className="text-white">{new Date(maintenance.endTime).toLocaleTimeString()}</strong></span>
            </div>
          )}
          ...
          <p className="text-[10px] text-slate-500 font-mono">
            Floyd School Offline Ecosystem • Central Super Admin Control
          </p>
        </div>
      </div>
    );
