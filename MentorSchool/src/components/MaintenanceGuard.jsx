...
export default function MaintenanceGuard({ children }) {
  ...
  const checkStatus = async () => {
    setChecking(true);
    try {
      const res = await api.get('/public/maintenance-status?portal=mentorSchool');
        ...
        <div className="max-w-md w-full bg-slate-800/80 border border-amber-500/30 rounded-2xl p-8 shadow-2xl backdrop-blur-md space-y-6">
          ...
          <div className="space-y-2">
            <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-amber-500/10 text-amber-300 text-xs font-semibold">
              ...
              <span>Mentor Operations Maintenance</span>
            </div>
            <h1 className="text-xl font-bold tracking-tight text-white">
              Mentor Portal Temporarily Offline
            </h1>
            <p className="text-xs text-slate-300 leading-relaxed">
              {maintenance.message || 'Mentor classroom operations are undergoing scheduled maintenance and system optimization.'}
            </p>
          </div>
          ...
          {maintenance.endTime && (
            <div className="bg-slate-900/60 border border-slate-700/60 rounded-xl p-3 text-xs text-slate-300 flex items-center justify-center gap-2">
              ...
              <span>Expected operational by: <strong className="text-white">{new Date(maintenance.endTime).toLocaleTimeString()}</strong></span>
            </div>
          )}
          ...
          <p className="text-[10px] text-slate-500 font-mono">
            Floyd School Field Operations • Central Super Admin Control
          </p>
        </div>
      </div>
    );
