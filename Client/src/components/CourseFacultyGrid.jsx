...
const CourseFacultyCard = React.memo(({ mentor, index, variant }) => {
    ...
    return (
        <motion.div
            ...
            className={`snap-center flex-shrink-0 w-full ${isMobile ? 'min-h-[480px]' : 'aspect-[3/4]'} rounded-[2.5rem] overflow-hidden border-2 transition-all duration-700 flex flex-col items-center p-8 md:p-10 gap-8 relative cursor-default group
                ${isDark 
        ...
        >
            ...
            <div className="flex-grow flex flex-col items-center text-center min-w-0 relative z-10 w-full">
                <div className="space-y-2 mb-6 flex flex-col items-center">
                    <h3 className={`text-xl md:text-3xl font-black tracking-tight uppercase leading-none transition-colors w-full break-words
                        ${isDark ? 'text-white group-hover:text-blue-500' : 'text-slate-900 group-hover:text-blue-600'}`}>
                        {mentor.name}
                    </h3>
                    <p className={`font-black text-[12px] md:text-[13px] tracking-[0.2em] uppercase pb-4 opacity-80
