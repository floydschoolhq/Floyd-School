...
const CommentSection = ({ moduleId, moduleTitle }) => {
    ...
    if (loading) return <div className="p-8 text-center text-slate-400 font-bold animate-pulse uppercase tracking-widest text-[10px]">Synchronizing Discussion Thread...</div>;
    ...
    return (
        <div className="mt-12 space-y-8 font-['Inter']">
            <div className="flex items-center gap-3">
                ...
                <span className="bg-slate-100 text-slate-500 px-3 py-1 rounded-full text-[10px] font-black">{comments.length} Thoughts</span>
            </div>
            ...
            <div className="space-y-6">
                            ...
                            <div className="flex gap-4">
                                ...
                                <div className="flex-1">
                                    <div className="flex items-center justify-between mb-2">
                                        <h4 className="font-bold text-slate-900 text-sm">{comment.studentName}</h4>
                                        <span className="text-[10px] font-black text-slate-300 uppercase tracking-widest">
                                            {new Date(comment.createdAt).toLocaleDateString()}
                                        </span>
                                    </div>
                                    <p className="text-sm text-slate-600 leading-relaxed mb-4">{comment.text}</p>

                                    <div className="flex items-center gap-6">
                                        {comment.status === 'resolved' ? (
                                            <button
                                                onClick={() => handleConfirmResolution(comment._id)}
                                                className="flex items-center gap-2 text-[10px] font-black text-emerald-500 hover:text-emerald-600 uppercase tracking-widest bg-emerald-50 px-3 py-1.5 rounded-full border border-emerald-100 transition-all"
                                            >
                                        ...
                                        <div className="mt-6 ml-4 pl-6 border-l-2 border-slate-50 space-y-4">
                                            {comment.replies.map((reply, ridx) => (
                                                <div key={ridx} className="flex gap-3">
                                                    <div className="w-8 h-8 rounded-lg bg-slate-50 flex items-center justify-center text-slate-300 font-black text-[10px]">
                                                        {reply.userName[0]}
                                                    </div>
                                                    <div>
                                                        <div className="flex items-center gap-2 mb-1">
                                                            <h5 className="font-bold text-slate-800 text-xs">{reply.userName}</h5>
                                                            {reply.userRole && (reply.userRole === 'mentor' || reply.userRole === 'admin') && (
                                                                <span className="bg-amber-100 text-amber-600 text-[8px] px-1.5 py-0.5 rounded-md font-black uppercase tracking-widest border border-amber-200">
                                                                    Official
                                                                </span>
                                                            )}
                                    ...
                                    {replyTo === comment._id && (
                                        ...
                                        >
                                            <input
                                                ...
                                                className="flex-1 bg-slate-50 border border-slate-100 rounded-xl p-3 text-xs font-medium outline-none focus:border-sky-500"
                                                autoFocus
                                            />
                                            <button
                                                onClick={() => handleReply(comment._id)}
                                                className="bg-sky-500 text-white px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-sky-600 transition-all"
                                            >
                                                Reply
                                            </button>
                                        </motion.div>
                ...
                {comments.length === 0 && (
                    <div className="text-center py-12 bg-slate-50/50 rounded-[2rem] border-2 border-dashed border-slate-100">
                        ...
                        <p className="text-slate-400 font-black uppercase tracking-widest text-[10px]">No discussions started for this node</p>
                        <p className="text-slate-300 text-[10px] font-bold mt-1 uppercase italic tracking-tighter">Be the first to share an insight.</p>
                    </div>
                )}
            </div>
        </div>
